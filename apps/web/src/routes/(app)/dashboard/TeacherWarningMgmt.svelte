...
<div class="flex items-center gap-2">
	<button
		...
		on:click={() => {
			selectedDate = selectedDateTime
				.minus({
					days: selectedDateTime.weekday === 7 ? 3 : selectedDateTime.weekday === 6 ? 2 : 1,
				})
				.toISODate()
	...
	<button
		...
		on:click={() => {
			selectedDate = selectedDateTime
				.plus({
					days: selectedDateTime.weekday === 4 ? 3 : selectedDateTime.weekday === 5 ? 2 : 1,
				})
				.toISODate()
											...
											>
												...
												<div class="flex flex-wrap items-center gap-4">
													{#each warningCasesShort as warning}
														{@const disabled =
															pendingWarning ||
															(warnings[student.id]?.includes('Absent') && warning !== 'Absent')}
														<div class="flex items-center">
															<input
																on:change={() => {
																	student_id = student.id
																	if (warning === 'Absent') {
																		warnings[student.id] = warnings[student.id].includes('Absent')
																			? []
																			: ['Absent']
																	}
																}}
