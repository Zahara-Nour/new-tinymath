<script lang="ts">
	...
	import IconPencil from '$lib/ui/icons/IconPencil.svelte'
	import IconCheck from '$lib/ui/icons/IconCheck.svelte'
	import IconNew from '$lib/ui/icons/IconNew.svelte'
	...
	function updatePost(post_id: number | null, posts: Post[]) {
		...
		if (post_id) {
			...
			if (post) {
				...
				updated_at = DateTime.fromSQL(post.updated_at.replace(/T/, ' ')).toISODate()
				published_at = DateTime.fromSQL(post.published_at.replace(/T/, ' ')).toISODate()
			}
		} else {
			resetPost()
		}
		...
		<section class="p-4">
			<form method="post" use:enhance={submitPost}>
				...
				<div class="flex justify-center">
					...
					{:else}
						<div class="inline-block p-1" use:popup={popupUpdatePost}>
							<button type="submit" disabled={false} class="btn variant-filled-primary my-4">
