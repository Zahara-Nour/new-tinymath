<script lang="ts">
	...
	import IconScan from '$lib/ui/icons/IconScan.svelte'
	...
	let colorResult: typeof $correct_color | typeof $incorrect_color | typeof $unoptimal_color
...
z-index:100"
	...
	<div class="my-3 flex justify-end">
		<button on:click={toggleDetails} class="btn-icon variant-filled-primary"><IconScan /></button>
	</div>

	{#if classroom}
		<div class="flex w-full justify-around" style="overflow-x:auto;">
			<div class="w-full">
		...
		<div class={'flex items-center justify-around  p-2'} style={`background:${colorResult}`}>
			<div class="flex h-full flex-col items-center justify-around">
				...
				<a href={`/automaths${query}`}>
					<button class="btn-icon my-2 bg-white text-2xl text-black"><IconHome /></button>
				</a>
			</div>
