...
export const load: LayoutServerLoad = async ({ locals: { getSession, supabase, adminAuth } }) => {
	const session = await getSession()
	const errors: string[] = []
	const FETCH_USER_ERROR = 'Erreur lors de la récupération des données utilisateur.'
	...
	const FETCH_PLAYER_ERROR = 'Erreur lors de la récupération des joueur navadra.'
	const NOT_CREATED_ERROR = "Le compte n'existe pas. Il faut demander à M. Le Jolly de le créer."
	const USER_UPDATE_FAILED = "Les données utilisateurs n'ont pu être mises à jour."
	...
	if (session) {
		...
		if (userError) {
			...
			if (isTeacherProfile(profile) || isStudentProfile(profile)) {
				const { data: userClassesData, error: userClassesError } = await DB_fetchUserClasses(
					supabase,
					profile.id,
				)

				if (userClassesError || !userClassesData) {
					console.log(userClassesError?.message || 'no data returned for user classes')
			...
			if (isTeacherProfile(profile)) {
				...
				const { data: studentsData, error: studentsError } = await DB_fetchTeacherStudents(
					supabase,
					profile.id,
				)
			...
			if (isAdminProfile(profile)) {
				const { data: schoolsData, error: schoolsError } = await DB_fetchSchools(supabase)
				if (schoolsError) {
