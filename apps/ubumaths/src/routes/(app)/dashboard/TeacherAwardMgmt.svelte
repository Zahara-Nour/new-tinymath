<script lang="ts">
	...
	import IconCards from '$lib/ui/icons/IconCards.svelte'
	...
	async function addGidouille(student: StudentProfile) {
		...
		const { error } = await DB_updateStudentGidouille(db, student.id, student.gidouilles + 1)
		if (error) {
	...
	async function useCard(student: StudentProfile) {
		...
		if (selectedCard.effect) {
			...
			if (selectedCard.effect.name === 'draw') {
				const { error, draws } = await drawVipCards(student, selectedCard.effect.param, 0)
				if (error) {
					toastStore.trigger({
...
<div>
	...
	<button class="btn-icon variant-filled-primary" disabled={granting} on:click={grantGidouilles}>
		<Gidouille />
	</button>
</div>
		...
		<section class="p-4">
			...
			<ul class="list mt-4">
				{#each students[selectedClasseId].sort( (a, b) => a.firstname.localeCompare(b.firstname), ) as student}
					{@const studentCard = student.vips.find(({ card }) => card.name == selectedCard.name)}

					<li
						class="bg-surface-500 flex w-full max-w-full items-center justify-between rounded-md p-2 text-black shadow-md"
					>
						...
						<div class="flex flex-wrap items-center justify-center gap-4">
							...
							<div class="flex flex-wrap items-center justify-center gap-1">
								...
								<button
									class="btn-icon variant-filled-primary"
									disabled={pendingGidouille[student.id]}
									on:click={() => (selectedStudent = selectedStudent === student ? null : student)}
								>
									<IconCards />
								</button>
							</div>
