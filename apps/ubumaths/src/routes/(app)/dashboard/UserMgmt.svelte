<script lang="ts">
	...
	$: selectedSchoolId = schools && schools.length > 0 ? schools[selectedSchool].id : 0
	...
	async function getTeachers(school_id: number) {
		const { data: teachersData, error: teachersError } = await DB_fetchSchoolTeachers(db, school_id)
		teachers = []
		if (teachersError) {
			...
			teachers = teachersData.map((teacher) => cleanProfile(teacher) as TeacherProfile)
		}
	}
	async function getStudents(classe_indexes: number[]) {
		...
		const promises = classe_indexes
			...
			.map((classe_index) => DB_fetchClasseStudents(db, classes[classe_index].id))
	...
	async function getClasses(school_id: number) {
		...
		const { data: classesData, error: classesError } = await DB_fetchSchoolClasses(db, school_id)
		classes = []
		if (classesError) {
		...
		if (text) {
			rows = text.split('\n').map((row) => {
				...
				return {
					...
					teacher_id: teachers.find((teacher) => teacher.email === cells[5])?.id,
					teacher_uuid: teachers.find((teacher) => teacher.email === cells[5])?.auth_id,
					gidouilles: 0,
					vips: JSON.stringify({}),
				}
			})
	...
	<section class="p-4">
		<select class="select" size="1" bind:value={selectedSchool}>
			{#each schools as school, i}
				{@const name = school.country + ' - ' + school.city + ' - ' + school.name}
				...
				<svelte:fragment slot="content">
					<ListBox multiple>
						{#each teachers as teacher}
							<ListBoxItem bind:group={selectedTeachers} name="teachers" value={teacher.id}
								...
								{teacher.classe_ids
									...
									.join(', ')}
								<svelte:fragment slot="trail">trail</svelte:fragment></ListBoxItem
							>
						{/each}
					</ListBox>
				</svelte:fragment>
							...
							{#if students[classe_id]}
								{#each students[classe_id] as student}
									<ListBoxItem
										...
										value={student.id}
										><svelte:fragment slot="lead">{classes[classe_index].name}</svelte:fragment>
	...
	<section class="p-4">
		<form method="post" use:enhance={submitAddClass}>
			...
			<div class="flex justify-center">
				...
				{:else}
					<div class="inline-block p-1" use:popup={popupAddClass}>
						<button
							disabled={!schools ||
								!schools.length ||
								!className ||
							...
							class="btn variant-filled-tertiary my-4"
						>
							Créer la classe
						</button>
					</div>
	...
	<section class="p-4">
		<form method="post" use:enhance={submitAddTeacher}>
			...
			<div class="flex justify-center">
				...
				{:else}
					<button
						disabled={teachers.some((teacher) => teacher.email === teacherEmail) ||
	...
	<section class="p-4">
		<form method="post" use:enhance={submitAddStudents}>
							...
							<tbody>
								{#each rows as row}
									<tr>
										...
										<td>{row.lastname}</td>
										<td>{row.grade}</td>
										<td>{row.classe_ids?.map((id) => classes.find((c) => c.id === id)?.name)}</td>
										<td>{teachers.find((teacher) => teacher.id === row.teacher_id)?.email}</td>
									</tr>
								{/each}
							</tbody>
						</table>
			...
			<div class="flex justify-center">
				...
				{:else}
					<button disabled={!textarea} class="btn variant-filled-tertiary my-4">
						Ajouter les élèves
					</button>
				{/if}
			</div>
