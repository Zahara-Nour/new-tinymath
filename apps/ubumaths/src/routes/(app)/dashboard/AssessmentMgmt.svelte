<script lang="ts">
	...
	import type { Assessment, Basket, School, StudentProfile, Teacher } from '../../../types/type'
	...
	function getAllStudents(classesIds: number[], studentsByClasse: Record<number, number[]>) {
		allStudents = []
		// whole classes
		...
		const indivuals = objectMap(studentsByClasse, (students_ids, classe_id) =>
			students_ids
				.map(
					(student_id) =>
						u.students[parseInt(classe_id, 10)].find((student) => student.id === student_id)!,
				)
				.filter((student) => student && !allStudents.some((s) => s.id === student.id)),
		)
...
{#if assessments.length}
	<div class="card h-80 overflow-auto p-4">
		{#each assessments as assessment}
			<div class="bg-surface-500 mb-4 flex flex-col rounded-md p-4 text-black shadow-md">
	...
	<div class="card p-4">
		<label class="label">
			<div class="flex items-center">
				...
				<select class="select ml-2" bind:value={selectedClasseId}>
					{#each u.classes as classe}
