<script lang="ts">
	import questions, { getQuestion, questions_ids } from '$lib/questions/questions'
	...
	function generateExoTexmacs() {
		...
		questions.forEach((q) => {
			...
			if (q.enounceAlone) {
				enounce = '<\\exo|'
				enounce += question.enounces[0].replace(/\$\$(.*)?\$\$/g, replaceTexmacs)
				enounce += '>\n'
				enounce += '<\\enumerate-alpha>\n'
				solution = '<\\exo|'
				solution += question.enounces[0].replace(/\$\$(.*)?\$\$/g, replaceTexmacs)
				solution += '>\n'
				solution += '<\\enumerate-alpha>\n'
				for (let i = 0; i < q.count; i++) {
					const generated = generateQuestion(question, generateds, q.count, offset)
					const correctedQuestion = assessItem(prepareAnsweredQuestion(generated))
					enounce += '<item>'
					solution +=
						'<item>' +
						correctedQuestion.simpleCorrection.map((line) => line.texmacs).join('') +
						'\n\n'
					if (correctedQuestion.expression) {
						enounce += '<math|' + math(correctedQuestion.expression).texmacs + '>\n\n'
					}
				...
				for (let i = 0; i < q.count; i++) {
					...
					const correctedQuestion = assessItem(prepareAnsweredQuestion(generated))
					solution +=
						'\\item ' + correctedQuestion.simpleCorrection.map((line) => line.texmacs).join(' ')
					enounce +=
						'\\item ' +
	...
	function changeDomainSubdomainLevel(new_domain: string, subdomain = '', level = 0) {
	...
	function changeSubdomainLevel(new_subdomain: string, new_level: number) {
		subdomain = subdomains.includes(new_subdomain) ? new_subdomain : subdomains[0]
	...
	function generateExemple(theme: string, domain: string, subdomain: string, level: number) {
		const q = getQuestion(theme, domain, subdomain, level)
	...
	function copyLink() {
		...
		const base = dev ? 'http://localhost:5173/' : 'https://ubumaths.fr/'
...
<div class="container mx-auto px-4 pt-2">
	...
	{:else if theme}
		...
		>
			{#each themes as t (t)}
				<Tab on:click={() => changeThemeDomainSubdomainLevel(t)} group={theme} name={t} value={t}
					>{t}</Tab
				>
			{/each}
			<svelte:fragment slot="panel">
				<Accordion autocollapse>
					{#each domains as d, i (theme + d)}
						<AccordionItem open={d === domain} on:click={() => changeDomainSubdomainLevel(d)}>
							<svelte:fragment slot="summary"
								><p class={'font-bold ' + (domain === d ? 'text-primary-500' : '')}>
									{@html $formatLatexToHtml(d)}
								</p></svelte:fragment
							>
							<div slot="content" class="border-primary-500 border-l-2 pl-14">
	...
	{#if displayExemple}
		<div
			class=" bg-surface-50-900-token sticky bottom-0 z-10 flex w-full items-center justify-center border-t-2 px-2 py-5"
		>
			<hl />
			<QuestionCard
				class="max-w-xl"
