...
export function DB_insertUser(supabase: SupabaseClient<Database>, profile: UserProfile) {
...
export function DB_updateUser(supabase: SupabaseClient<Database>, profile: UserProfile) {
	const row = isStudentProfile(profile)
		? {
				...profile,
				vips: JSON.stringify(convertArrayWalletToObjectWallet(profile.vips)),
...
export function DB_fetchUser(supabase: SupabaseClient<Database>, user_id: string) {
	return supabase
		.from('users')
		.select(
...
export function DB_fetchUserByEmail(supabase: SupabaseClient<Database>, email: string) {
	return supabase
		.from('users')
		.select(
			'id, firstname, lastname, role, email, school_id, teacher_id, grade, auth_id, classe_ids, gidouilles, vips',
...
export async function DB_fetchUserClasses(supabase: SupabaseClient<Database>, user_id: number) {
...
export async function DB_fetchStudentPendingAssignments(
	...
	return supabase
		.from('assignments')
		.select('id, teacher_id, student_id, status, total, mark, questions, basket, title')
...
export async function DB_fetchDayTeacherStudents(
	...
	if (request.error) {
		...
		const classe_ids = request.data.filter((c) => c.schedule.includes(day)).map((c) => c.id)
		return supabase
			.from('users')
			.select('*')
...
export async function DB_fetchSchoolClasses(supabase: SupabaseClient<Database>, school_id: number) {
	return supabase
		.from('classes')
...
export async function DB_fetchSchoolTeachers(
	...
	return supabase
		.from('users')
		.select('auth_id, role, email, firstname, lastname, id, classe_ids, school_id')
		.eq('role', 'teacher')
		.eq('school_id', school_id)
}
...
export async function DB_fetchPlayer(supabase: SupabaseClient<Database>, user_id: number) {
	return supabase
...
export async function DB_fetchMonster(supabase: SupabaseClient<Database>, monster_id: number) {
	return supabase
...
export async function DB_fetchPost(supabase: SupabaseClient<Database>, post_id: number) {
	return supabase
		.from('posts')
		.select('id, title, content, tags, summary, metadescription, updated_at, published_at')
		.eq('id', post_id)
		.maybeSingle()
}
...
export async function DB_fetchPostsByTags(supabase: SupabaseClient<Database>, tags: string[]) {
