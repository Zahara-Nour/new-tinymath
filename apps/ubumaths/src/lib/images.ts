...
const { info, fail, warn } = getLogger('images', 'info')
import { browser } from '$app/environment'

import db from '$lib/db'

export async function fetchImage(path: string, bucket = 'public/mental'): Promise<string> {
		...
		if (!base64) {
			...
			if (error) {
				...
				return new Promise(function (resolve, reject) {
					...
					reader.onerror = () => reject(new Error(`Error while loading image ${path}`))
