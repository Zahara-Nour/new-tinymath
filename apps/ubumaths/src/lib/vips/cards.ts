...
async function useVipCard(card: VipCard, student: StudentProfile) {
	...
	if (result.error) {
		...
		return new Error("La carte VIP n'a pas pu être utilisée : " + result.error.message)
	} else {
...
async function addVipCard(card: VipCard, student: StudentProfile) {
	...
	if (result.error) {
		...
		return new Error("La carte VIP n'a pas pu être ajoutée : " + result.error.message)
	} else {
		student.vips = newWallet
		return null
	}
...
async function drawVipCards(student: StudentProfile, n = 1, gidouilles = 3) {
	...
	for (let i = 0; i < n; i++) {
		...
		const rarity =
			alea < 0.5 ? 'common' : alea < 0.85 ? 'uncommon' : alea < 0.95 ? 'rare' : 'legendary'
