<script lang="ts">
	...
	import { isTeacher, type StudentProfile, type User, type VipCard } from '../../types/type'
	import {
		getModalStore,
		getToastStore,
		type ModalSettings,
	...
	async function onUseCard(card: VipCard) {
		...
		if (card.effect) {
			...
			if (card.effect.name === 'draw') {
				const { error, draws } = await drawVipCards(student, card.effect.param, 0)
				if (error) {
					toastStore.trigger({
...
>
	...
	<div class="flex h-2/3 flex-col items-center justify-start p-2">
		<div
			class="text-primary-500 my-2 h-auto w-full text-center text-2xl font-bold"
		...
		{#if isTeacher(user)}
			<div class="flex justify-center gap-4">
				<button class="btn variant-filled-primary mt-2" {disabled} on:click={() => onUseCard(card)}>
					Utiliser
				</button>
				<button class="btn-icon variant-filled-error mt-2" on:click={() => onRemoveCard(card)}>
					<IconMult />
				</button>
			</div>
		{/if}
