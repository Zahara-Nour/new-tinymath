...
export const formatLatexToHtml = writable((o: FormatToHtmlArg): NonNullable<FormatToHtmlArg> => '')
	...
	if (!get(mathliveReady) && browser) {
		import('@repo/tinymathlive')
			.then((m) => {
				...
				m.MathfieldElement.decimalSeparator = ','
				...
				const _formatToHtml = (o: FormatToHtmlArg): NonNullable<FormatToHtmlArg> => {
					if (!o) {
						return ''
					}
