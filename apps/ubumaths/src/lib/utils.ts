...
import type { FormatToLatexArg, FormatToTexmacsArg, ObjectWithText } from '../types/type'
// loggers
...
export const formatToLatex = (o: FormatToLatexArg): NonNullable<FormatToLatexArg> => {
	if (!o) {
		return ''
...
export const formatToTexmacs = (o: FormatToTexmacsArg): NonNullable<FormatToTexmacsArg> => {
	if (!o) {
		return ''
	}
	if (Array.isArray(o)) {
...
export function objectMap<S, T>(
	...
	return Object.keys(object).reduce(
		function (result: { [index: string]: T }, key, idx) {
			...
			if (
				!(
					(typeof new_value === 'object' && isEmptyObject(new_value as object)) ||
