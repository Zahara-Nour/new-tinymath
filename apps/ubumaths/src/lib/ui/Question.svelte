			...
			: isQuestionFillIn(question)
				...
				: isQuestionResultOrRewrite(question)
					? question.expression_latex +
						'=' +
						(question.answerFormat_latex || '\\ldots').replace(/\\ldots/g, addPlaceholder)
...
<div class={`flex flex-col items-center justify-around ${$$props.class}`}>
	{#each question.order_elements as element}
		{#if element === 'enounce' && enounce}
			<div
				id="enounce"
				class={(correction ? 'text-tertiary-500 text-base' : '') +
					' max-w-4xl text-center leading-normal'}
			>
				{@html enounce}
			</div>
		...
		{:else if !correction && element === 'choices' && isQuestionChoice(question)}
			<div class="mt-3 flex flex-wrap justify-around" style={`font-size:${magnify_2xl};`}>
				{#each question.choices as choice, i}
					<button
						class={'m-2  rounded-lg border-4 p-2 ' +
							(interactive && answers.includes(i) ? 'border-primary-500' : 'border-tertiary-500')}
						style={'min-width:2em'}
						on:click={() => onChoice(i)}
					>
						{#if choice.image}
	...
	{#if !correction && interactive && (isQuestionChoices(question) || nfields > 1) && immediateCommit}
		<button
			...
			class="variant-filled-primary mt-3 p-1">Valider</button
		>
	{/if}
	...
	{#if correction}
		<div class="mt-3" style={`font-size:${magnify_2xl};`}>
			{#each simpleCorrection as line}
				<div class=" relative z-0 my-1" style={`word-break: break-word ;white-space: normal;`}>
					<CorrectionLine {line} />
				</div>
			{/each}
		</div>
