  ...
  return {
    ...
    isLowerThan(exp: Node | string | number | Decimal) {
      ...
      try {
        ...
        result = e1.eval({ decimal: true }).isLowerThan(e2.eval({ decimal: true }));
      }
      return result;
    },
    ...
    equals(exp: Node | string | number | Decimal) {
      ...
      if (isEquality(this)) {
        ...
        return (
          (isInequalityLess(e) && this.first.equals(e.first) && this.last.equals(e.last)) ||
          (isInequalityMore(e) && this.first.equals(e.last) && this.last.equals(e.first))
        );
      } else if (isInequalityLessOrEqual(this)) {
        return (
          (isInequalityLessOrEqual(e) && this.first.equals(e.first) && this.last.equals(e.last)) ||
          (isInequalityMoreOrEQual(e) && this.first.equals(e.last) && this.last.equals(e.first))
        );
      } else if (isInequalityMore(this)) {
        return (
          (isInequalityMore(e) && this.first.equals(e.first) && this.last.equals(e.last)) ||
          (isInequalityLess(e) && this.first.equals(e.last) && this.last.equals(e.first))
        );
      } else if (isInequalityMoreOrEQual(this)) {
        return (
          (isInequalityMoreOrEQual(e) && this.first.equals(e.first) && this.last.equals(e.last)) ||
          (isInequalityLessOrEqual(e) && this.first.equals(e.last) && this.last.equals(e.first))
        );
      } else {
    ...
    isNumeric() {
      return (
        isNumber(this) ||
        (isExpressionWithChildren(this) && this.children.every((child) => child.isNumeric()))
      );
    },
   ...
   */
    ...
    eval(params?: EvalArg) {
      ...
      if (this.isNumeric() && (isMaxP(this) || isMinP(this))) {
        ...
        if (unit && n.unit) {
          if (
            ...
            throw new Error(
              `Unités incompatibles ${n.string} ${typeof unit === 'string' ? unit : unit.string}`
            );
          }
          if (unit !== 'HMS') {
      ...
      if (params.decimal && unit !== 'HMS') {
        ...
        e = number(evaluate(e, params).toDecimalPlaces(params.precision).toString());
    ...
    shallow() {
      return {
        nature: this.type,
        children: isExpressionWithChildren(this) ? this.children.map((e) => e.type) : null,
    ...
    matchTemplate(t: Node) {
      ...
      function checkChildren(e: ExpressionWithChildren, t: ExpressionWithChildren) {
      ...
      function checkDigitsNumber(n: number, minDigits: number, maxDigits: number) {
      ...
      if (isNumber(t)) {
        ...
        switch (t.nature) {
          ...
          case '$ep':
          case '$ei':
            if (!t.relative && this.isOpposite()) return false;
            if (
              (t.signed && (isOpposite(this) || isPositive(this))) ||
              (t.relative && isOpposite(this))
            )
              return this.first.matchTemplate(template({ nature: t.nature, children: t.children }));
            if (
              !isHole(t.children[1]) &&
              !checkDigitsNumber(
                (this as Numbr).value.toNumber(),
                !isHole(t.children[0]) ? (t.children[0] as Numbr).value.toNumber() : 0,
          ...
          case '$d':
            if (t.relative && isOpposite(this)) return this.first.matchTemplate(t);
            ...
            if (this.isInt()) {
              ...
              const [integerPartString, decimalPartString] = (this as Numbr).value
                .toString()
                .split('.');
              ...
              if (t.children[1].isTemplate()) {
                if (!number(Math.floor(Math.log10(decimalPart)) + 1).matchTemplate(t.children[1]))
                  return false;
              } else if (
                !checkDigitsNumber(
                  decimalPart,
...
export function inequality(children: Node[], relation: '<' | '>' | '<=' | '>=') {
  if (relation === '<') {
    return createNode({
