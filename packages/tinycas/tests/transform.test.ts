...
describe('Testing substitutions', () => {
  ...
  test.each(t)('substitution of %s by %s in %s', (symbol, value, e, expected) => {
...
describe('Testing derivating functions', () => {
  ...
  const t2 = [
    ...
    ['cos(x)', '-sin(x)'],
    ['sin(x)', 'cos(x)'],
    ['ln(cos(x))', '-cos(x)^{-1}*sin(x)'],
    ['ln(exp(x))', '1'],
    ['x*x', '2*x'],
    ['x*ln(x)', '1+ln(x)'],
    ['x/ln(x)', '-ln(x)^{-2}+ln(x)^{-1}'],
    ['x^2', '2*x'],
    ['x^3', '3*x^2'],
    ['6x^3-5x^2+10x-7', '10-10*x+18*x^2'],
    ['sqrt(x)', 'x^{-1/2}/2'],
    ['x^{1/2}', 'x^{-1/2}/2']
  ];

  test.each(t1)('%s equals to %s', (e, expected) => {
